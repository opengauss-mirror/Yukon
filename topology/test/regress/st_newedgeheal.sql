...
SELECT public.ST_NewEdgeHeal('city_data', 1, null);
SELECT public.ST_NewEdgeHeal('city_data', null, 1);
SELECT public.ST_NewEdgeHeal(null, 1, 2);
SELECT public.ST_NewEdgeHeal('', 1, 2);

-- Not connected edges
SELECT public.ST_NewEdgeHeal('city_data', 25, 3);

-- Other connected edges
SELECT public.ST_NewEdgeHeal('city_data', 9, 10);

-- Closed edge
SELECT public.ST_NewEdgeHeal('city_data', 2, 3);
SELECT public.ST_NewEdgeHeal('city_data', 3, 2);

-- Heal to self
SELECT public.ST_NewEdgeHeal('city_data', 25, 25);
...
SELECT 'MH(4,5)', public.ST_NewEdgeHeal('city_data', 4, 5);

-- Face and other edges involved, SQL/MM caseno 1, drops node 16
SELECT 'MH(21,6)', public.ST_NewEdgeHeal('city_data', 21, 6);
-- Face and other edges involved, SQL/MM caseno 2, drops node 19
SELECT 'MH(8,15)', public.ST_NewEdgeHeal('city_data', 8, 15);
-- Face and other edges involved, SQL/MM caseno 3, drops node 8
SELECT 'MH(12,22)', public.ST_NewEdgeHeal('city_data', 12, 22);
-- Face and other edges involved, SQL/MM caseno 4, drops node 11
SELECT 'MH(16,14)', public.ST_NewEdgeHeal('city_data', 16, 14);
...
SELECT public.CreateTopology('t') > 1;
CREATE TABLE t.f_lin(id varchar);
SELECT 'lin_layer', public.AddTopoGeometryColumn('t', 't', 'f_lin','g', 'LINE');
CREATE TABLE t.f_poi(id varchar);
SELECT 'poi_layer', public.AddTopoGeometryColumn('t', 't', 'f_poi','g', 'POINT');
CREATE TABLE t.f_mix(id varchar);
SELECT 'mix_layer', public.AddTopoGeometryColumn('t', 't', 'f_mix','g', 'COLLECTION');

SELECT 'N'||public.TopoGeo_AddPoint('t', 'POINT(2 8)');  -- 1
SELECT 'E'||public.AddEdge('t', 'LINESTRING(2 2, 2  8)');        -- 1
SELECT 'E'||public.AddEdge('t', 'LINESTRING(2  8,  8  8)');      -- 2
...
INSERT INTO t.f_mix VALUES ('F+N1',
  public.CreateTopoGeom('t', 4, 3, '{{1,1}}'));
...
INSERT INTO t.f_lin VALUES ('F+E1',
  public.CreateTopoGeom('t', 2, 1, '{{1,2}}'));
SELECT public.ST_NewEdgeHeal('t', 1, 2);
SELECT public.ST_NewEdgeHeal('t', 2, 1);
...
INSERT INTO t.f_mix VALUES ('F+E1',
  public.CreateTopoGeom('t', 4, 3, '{{1,2}}'));
SELECT public.ST_NewEdgeHeal('t', 1, 2);
SELECT public.ST_NewEdgeHeal('t', 2, 1);
WITH deleted AS ( DELETE FROM t.f_mix RETURNING g),
     clear AS ( SELECT ClearTopoGeom(g) FROM deleted)
...
SELECT public.ST_NewEdgeHeal('t', 1, 200);
SELECT public.ST_NewEdgeHeal('t', 100, 2);

-- Now see how signed edges are updated

SELECT 'tg-update', 'E'||public.AddEdge('t', 'LINESTRING(0 0, 5 0)');         -- 3
SELECT 'tg-update', 'E'||public.AddEdge('t', 'LINESTRING(10 0, 5 0)');        -- 4

INSERT INTO t.f_lin VALUES ('F+E3-E4',
  public.CreateTopoGeom('t', 2, 1, '{{3,2},{-4,2}}'));
INSERT INTO t.f_lin VALUES ('F-E3+E4',
  public.CreateTopoGeom('t', 2, 1, '{{-3,2},{4,2}}'));
...
-- 'F+E3-E4' and 'F-E3+E4'
SELECT 'tg-update', 'MH(3,4)', public.ST_NewEdgeHeal('t', 3, 4);
...
SELECT '#942', public.ST_NewEdgeHeal('t', 1, 5);



SELECT public.DropTopology('t');

-------------------------------------------------------------------------
-------------------------------------------------------------------------
-------------------------------------------------------------------------

SELECT '#1955', public.CreateTopology('t') > 1;

SELECT '#1955.1', 'E'||public.AddEdge('t', 'LINESTRING(0 0, 10 0, 10 10)');        -- 1
SELECT '#1955.1', 'E'||public.AddEdge('t', 'LINESTRING(0 0, 0 10, 10 10)'); ;      -- 2
...
SELECT '#1955.1', 'H:1,2', 'E' || public.ST_NewEdgeHeal('t', 1, 2), 'created';

SELECT '#1955.1', count(node_id), 'nodes left' as label FROM t.node GROUP BY label;

SELECT '#1955.2', 'E'||public.AddEdge('t', 'LINESTRING(50 0, 60 0, 60 10)');        -- 4
SELECT '#1955.2', 'E'||public.AddEdge('t', 'LINESTRING(50 0, 50 10, 60 10)'); ;     -- 5
SELECT '#1955.2', 'E'||public.AddEdge('t', 'LINESTRING(60 10, 70 10)'); ;           -- 6
...
-- another edge
SELECT '#1955.2', 'H:4,5', 'E' || public.ST_NewEdgeHeal('t', 4, 5), 'created';

SELECT '#1955.2', count(node_id), 'nodes left' as label FROM t.node GROUP BY label;

SELECT '#1955.3', 'E'||public.AddEdge('t', 'LINESTRING(80 0, 90 0, 90 10)');        -- 8
SELECT '#1955.3', 'E'||public.AddEdge('t', 'LINESTRING(80 0, 80 10, 90 10)'); ;     -- 9
SELECT '#1955.3', 'E'||public.AddEdge('t', 'LINESTRING(70 10, 80 0)'); ;            -- 10
...
-- another edge
SELECT '#1955.3', 'H:8,9', 'E' || public.ST_NewEdgeHeal('t', 8, 9), 'created';

SELECT '#1955.3', count(node_id), 'nodes left' as label FROM t.node GROUP BY label;

SELECT '#1955', public.DropTopology('t');
...
SELECT '#1998.-', public.DropTopology('t1998');
