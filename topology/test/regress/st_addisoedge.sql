...
-- null input
SELECT public.ST_AddIsoEdge('tt', 1, 2, NULL);
SELECT public.ST_AddIsoEdge(NULL, 1, 2, 'LINESTRING(0 0, 1 1)');
SELECT public.ST_AddIsoEdge('tt', 1, NULL, 'LINESTRING(0 0, 1 1)');
SELECT public.ST_AddIsoEdge('tt', NULL, 2, 'LINESTRING(0 0, 1 1)');

-- invalid curve
SELECT public.ST_AddIsoEdge('tt', 1, 2, 'POINT(0 0)');

-- non-simple curve
SELECT public.ST_AddIsoEdge('tt', 1, 2, 'LINESTRING(0 0, 10 0, 5 5, 5 -5)');

-- non-existing nodes
SELECT public.ST_AddIsoEdge('tt', 10000, 2, 'LINESTRING(0 0, 1 1)');

-- Curve endpoints mismatch
SELECT public.ST_AddIsoEdge('tt', 1, 2, 'LINESTRING(0 0, 1 1)');
SELECT public.ST_AddIsoEdge('tt', 1, 2, 'LINESTRING(0 1, 10 0)');

-- Node crossing
SELECT public.ST_AddIsoEdge('tt', 1, 2, 'LINESTRING(0 0, 10 0)');

-- Good one
SELECT 'E' || public.ST_AddIsoEdge('tt',
...
SELECT public.ST_AddIsoEdge('tt',
...
SELECT public.ST_AddIsoEdge('tt', 6, 7, 'LINESTRING(20 10, 30 10)');
