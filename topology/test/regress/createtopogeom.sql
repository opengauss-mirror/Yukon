...
SELECT public.CreateTopology('MiX') > 0;

-- Fails due to missing layer 1
SELECT public.CreateTopoGeom(
    'MiX', -- Topology name
...
SELECT 'l1', public.AddTopoGeometryColumn('MiX', 'MiX', 'poi', 'feat', 'POINT');

-- A Layer of type 1 (POINT) cannot contain a TopoGeometry of type 2 (LINE)
SELECT public.CreateTopoGeom( 'MiX', 2, 1, '{{12,2}}');
-- A Layer of type 1 (POINT) cannot contain a TopoGeometry of type 3 (POLY)
SELECT public.CreateTopoGeom( 'MiX', 3, 1, '{{13,3}}');
-- A Layer of type 1 (POINT) cannot contain a TopoGeometry of type 4 (COLL.)
SELECT public.CreateTopoGeom( 'MiX', 4, 1, '{{12,2}}');

-- Node 78 does not exist in topology MiX (trigger on "relation" table)
SELECT public.CreateTopoGeom( 'MiX', 1, 1, '{{78,1}}');

SELECT 'n1',  public.addNode('MiX', 'POINT(0 0)');

-- Success !
SELECT layer_id(tg), id(tg), type_(tg) FROM (
 SELECT public.CreateTopoGeom( 'MiX', 1, 1, '{{1,1}}') as tg
) foo;
