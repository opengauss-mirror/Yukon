...
-- Invalid topologies
select public.st_createtopogeo('', 'GEOMETRYCOLLECTION(POINT(0 0))');
select public.st_createtopogeo('t', 'GEOMETRYCOLLECTION(POINT(0 0))');
select public.st_createtopogeo(null, 'GEOMETRYCOLLECTION(POINT(0 0))');
...
-- Invalid geometries
select null from ( select public.CreateTopology('t', 4326) > 0 ) as ct;
select public.st_createtopogeo('t', null); -- Invalid geometry
select 'invalid_srid', public.st_createtopogeo('t', 'POINT(0 0)');
...
SELECT g, public.st_createtopogeo('t', g) FROM ( SELECT
'POINT(0 0)'
