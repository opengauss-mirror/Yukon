 ...
 * liblwgeom versions
 */
#define LIBLWGEOM_VERSION "3.2.1"
#define LIBLWGEOM_VERSION_MAJOR "3"
#define LIBLWGEOM_VERSION_MINOR "2"
#define LIBLWGEOM_GEOS_VERSION "31004"
...
*/
extern "C" int32_t clamp_srid(int32_t srid);
...
*/
extern "C" void lwgeom_set_handlers(lwallocator allocator,
        lwreallocator reallocator, lwfreeor freeor, lwreporter errorreporter,
        lwreporter noticereporter);

extern "C" void lwgeom_set_debuglogger(lwdebuglogger debuglogger);
 ...
 *
 */
extern "C" void lwgeom_request_interrupt(void);

/**
 * Cancel any interruption request
 */
extern "C" void lwgeom_cancel_interrupt(void);
...
extern "C" lwinterrupt_callback *lwgeom_register_interrupt_callback(lwinterrupt_callback *);
...
typedef struct 
{
	...
	int32_t srid;
	lwflags_t flags;
	uint8_t type; /* BEZIERTYPE */
	char pad[1];  /* Padding to 24 bytes (unused) */
} LWBEZIER;

/* Casts LWGEOM->LW* (return NULL if cast is illegal) */
extern "C" LWMPOLY *lwgeom_as_lwmpoly(const LWGEOM *lwgeom);
extern "C" LWMLINE *lwgeom_as_lwmline(const LWGEOM *lwgeom);
extern "C" LWMPOINT *lwgeom_as_lwmpoint(const LWGEOM *lwgeom);
extern "C" LWCOLLECTION *lwgeom_as_lwcollection(const LWGEOM *lwgeom);
extern "C" LWPOLY *lwgeom_as_lwpoly(const LWGEOM *lwgeom);
extern "C" LWLINE *lwgeom_as_lwline(const LWGEOM *lwgeom);

extern "C" LWCIRCSTRING *lwgeom_as_lwcircstring(const LWGEOM *lwgeom);
extern "C" LWCURVEPOLY *lwgeom_as_lwcurvepoly(const LWGEOM *lwgeom);
extern "C" LWCOMPOUND *lwgeom_as_lwcompound(const LWGEOM *lwgeom);
extern "C" LWPSURFACE *lwgeom_as_lwpsurface(const LWGEOM *lwgeom);
extern "C" LWTRIANGLE *lwgeom_as_lwtriangle(const LWGEOM *lwgeom);
extern "C" LWTIN *lwgeom_as_lwtin(const LWGEOM *lwgeom);
extern "C" LWGEOM *lwgeom_as_multi(const LWGEOM *lwgeom);
extern "C" LWGEOM *lwgeom_as_curve(const LWGEOM *lwgeom);

/* Casts LW*->LWGEOM (always cast) */
extern "C" LWGEOM *lwtin_as_lwgeom(const LWTIN *obj);
extern "C" LWGEOM *lwtriangle_as_lwgeom(const LWTRIANGLE *obj);
extern "C" LWGEOM *lwpsurface_as_lwgeom(const LWPSURFACE *obj);
extern "C" LWGEOM *lwmpoly_as_lwgeom(const LWMPOLY *obj);
extern "C" LWGEOM *lwmline_as_lwgeom(const LWMLINE *obj);
extern "C" LWGEOM *lwmpoint_as_lwgeom(const LWMPOINT *obj);
extern "C" LWGEOM *lwcollection_as_lwgeom(const LWCOLLECTION *obj);
extern "C" LWGEOM *lwcircstring_as_lwgeom(const LWCIRCSTRING *obj);
extern "C" LWGEOM *lwcompound_as_lwgeom(const LWCOMPOUND *obj);
extern "C" LWGEOM *lwcurvepoly_as_lwgeom(const LWCURVEPOLY *obj);
extern "C" LWGEOM *lwpoly_as_lwgeom(const LWPOLY *obj);
extern "C" LWGEOM *lwline_as_lwgeom(const LWLINE *obj);
extern "C" LWGEOM *lwpoint_as_lwgeom(const LWPOINT *obj);


extern "C" LWCOLLECTION* lwcollection_add_lwgeom(LWCOLLECTION *col, const LWGEOM *geom);
extern "C" LWMPOINT* lwmpoint_add_lwpoint(LWMPOINT *mobj, const LWPOINT *obj);
extern "C" LWMLINE* lwmline_add_lwline(LWMLINE *mobj, const LWLINE *obj);
extern "C" LWMPOLY* lwmpoly_add_lwpoly(LWMPOLY *mobj, const LWPOLY *obj);
extern "C" LWPSURFACE* lwpsurface_add_lwpoly(LWPSURFACE *mobj, const LWPOLY *obj);
extern "C" LWTIN* lwtin_add_lwtriangle(LWTIN *mobj, const LWTRIANGLE *obj);

extern "C" LWCOLLECTION* lwcollection_concat_in_place(LWCOLLECTION* col1, const LWCOLLECTION* col2);

/**
* Construct a new flags bitmask.
*/
extern "C" lwflags_t lwflags(int hasz, int hasm, int geodetic);
...
*/
extern "C" lwflags_t gserialized_get_lwflags(const GSERIALIZED *g);

/**
* Access to the float bounding box, if there is one.
* NULL if there is not.
*/
extern "C" const float * gserialized_get_float_box_p(const GSERIALIZED *g, size_t *ndims);

/**
* Extract the geometry type from the serialized form (it hides in
* the anonymous data area, so this is a handy function).
*/
extern "C" uint32_t gserialized_get_type(const GSERIALIZED *g);
...
*/
extern "C" uint32_t gserialized_max_header_size(void);
...
* boxes, etc.
*/
extern "C" int32_t gserialized_hash(const GSERIALIZED *g);

/**
* Extract the SRID from the serialized form (it is packed into
* three bytes so this is a handy function).
*/
extern "C" int32_t gserialized_get_srid(const GSERIALIZED *g);

/**
* Write the SRID into the serialized form (it is packed into
* three bytes so this is a handy function).
*/
extern "C" void gserialized_set_srid(GSERIALIZED *g, int32_t srid);
...
*/
extern "C" int gserialized_is_empty(const GSERIALIZED *g);

/**
* Check if a #GSERIALIZED has a bounding box without deserializing first.
*/
extern "C" int gserialized_has_bbox(const GSERIALIZED *gser);

/**
* Check if a #GSERIALIZED has a Z ordinate.
*/
extern "C" int gserialized_has_z(const GSERIALIZED *gser);

/**
* Check if a #GSERIALIZED has an M ordinate.
*/
extern "C" int gserialized_has_m(const GSERIALIZED *gser);

/**
* Check if a #GSERIALIZED is a geography.
*/
extern "C" int gserialized_is_geodetic(const GSERIALIZED *gser);

/**
* Return the number of dimensions (2, 3, 4) in a geometry
*/
extern "C" int gserialized_ndims(const GSERIALIZED *gser);
...
*/
extern "C" int gserialized_cmp(const GSERIALIZED *g1, const GSERIALIZED *g2);
...
* VARSIZE information.
*/
extern "C" GSERIALIZED* gserialized_from_lwgeom(LWGEOM *geom, size_t *size);
...
*/
extern "C" LWGEOM* lwgeom_from_gserialized(const GSERIALIZED *g);
...
*/
extern "C" int gserialized_get_gbox_p(const GSERIALIZED *g, GBOX *box);

/**
* Pull a #GBOX from the header of a #GSERIALIZED, if one is available. If
* it is not, return LW_FAILURE.
*/
extern "C" int gserialized_fast_gbox_p(const GSERIALIZED *g, GBOX *box);
...
*/
extern "C" GSERIALIZED *gserialized_set_gbox(GSERIALIZED *g, GBOX *gbox);

/**
* Remove the bounding box from a #GSERIALIZED. Returns a freshly
* allocated #GSERIALIZED every time.
*/
extern "C" GSERIALIZED* gserialized_drop_gbox(GSERIALIZED *g);

/**
* Return the serialization version
*/
extern "C" uint32_t gserialized_get_version(const GSERIALIZED *g);

/**
* Pull the first point values of a #GSERIALIZED. Only works for POINTTYPE
*/
extern "C" int gserialized_peek_first_point(const GSERIALIZED *g, POINT4D *out_point);
...
* will be released.
*/
extern "C" void lwgeom_drop_bbox(LWGEOM *lwgeom);
extern "C" void lwgeom_drop_srid(LWGEOM *lwgeom);
 ...
 */
extern "C" void lwgeom_add_bbox(LWGEOM *lwgeom);
/**
* Drop current bbox and calculate a fresh one.
*/
extern "C" void lwgeom_refresh_bbox(LWGEOM *lwgeom);
/**
* Compute a box for geom and all sub-geometries, if not already computed
*/
extern "C" void lwgeom_add_bbox_deep(LWGEOM *lwgeom, GBOX *gbox);
 ...
 */
extern "C" const GBOX *lwgeom_get_bbox(const LWGEOM *lwgeom);

/**
* Determine whether a LWGEOM can contain sub-geometries or not
*/
extern "C" int lwgeom_is_collection(const LWGEOM *lwgeom);
...
*/
extern "C" int lwtype_is_collection(uint8_t type);

/**
* Given an lwtype number, what homogeneous collection can hold it?
*/
extern "C" uint32_t lwtype_get_collectiontype(uint8_t type);

/**
* Return the type name string associated with a type number
* (e.g. Point, LineString, Polygon)
*/
extern "C" const char *lwtype_name(uint8_t type);
extern "C" uint8_t lwtype_multitype(uint8_t type);
 ...
 */
extern "C" POINT4D getPoint4d(const POINTARRAY *pa, uint32_t n);
 ...
 */
extern "C" int getPoint4d_p(const POINTARRAY *pa, uint32_t n, POINT4D *point);
 ...
 * NOTE: point is a real POINT3D *not* a pointer
 */
extern "C" POINT3DZ getPoint3dz(const POINTARRAY *pa, uint32_t n);
extern "C" POINT3DM getPoint3dm(const POINTARRAY *pa, uint32_t n);
 ...
 */
extern "C" int getPoint3dz_p(const POINTARRAY *pa, uint32_t n, POINT3DZ *point);
extern "C" int getPoint3dm_p(const POINTARRAY *pa, uint32_t n, POINT3DM *point);
 ...
 * NOTE: point is a real POINT3D *not* a pointer
 */
extern "C" POINT2D getPoint2d(const POINTARRAY *pa, uint32_t n);
 ...
 */
extern "C" int getPoint2d_p(const POINTARRAY *pa, uint32_t n, POINT2D *point);
 ...
 */
extern "C" void ptarray_set_point4d(POINTARRAY *pa, uint32_t n, const POINT4D *p4d);
...
*/
extern "C" POINTARRAY* ptarray_construct(char hasz, char hasm, uint32_t npoints);

/**
* Construct a new #POINTARRAY, <em>copying</em> in the data from ptlist
*/
extern "C" POINTARRAY* ptarray_construct_copy_data(char hasz, char hasm, uint32_t npoints, const uint8_t *ptlist);

/**
* Construct a new #POINTARRAY, <em>referencing</em> to the data from ptlist
*/
extern "C" POINTARRAY* ptarray_construct_reference_data(char hasz, char hasm, uint32_t npoints, uint8_t *ptlist);
...
* area.
*/
extern "C" POINTARRAY* ptarray_construct_empty(char hasz, char hasm, uint32_t maxpoints);
...
* not be added.
*/
extern "C" int ptarray_append_point(POINTARRAY *pa, const POINT4D *pt, int allow_duplicates);
 ...
 */
extern "C" int ptarray_append_ptarray(POINTARRAY *pa1, POINTARRAY *pa2, double gap_tolerance);

/**
* Insert a point into an existing #POINTARRAY. Zero
* is the index of the start of the array.
*/
extern "C" int ptarray_insert_point(POINTARRAY *pa, const POINT4D *p, uint32_t where);

/**
* Remove a point from an existing #POINTARRAY. Zero
* is the index of the start of the array.
*/
extern "C" int ptarray_remove_point(POINTARRAY *pa, uint32_t where);
 ...
 */
extern "C" POINTARRAY *ptarray_addPoint(const POINTARRAY *pa, uint8_t *p, size_t pdims, uint32_t where);
 ...
 */
extern "C" POINTARRAY *ptarray_removePoint(POINTARRAY *pa, uint32_t where);
 ...
 * @return #POINTARRAY is newly allocated
 */
extern "C" POINTARRAY *ptarray_merge(POINTARRAY *pa1, POINTARRAY *pa2);

extern "C" int ptarray_is_closed(const POINTARRAY *pa);
extern "C" int ptarray_is_closed_2d(const POINTARRAY *pa);
extern "C" int ptarray_is_closed_3d(const POINTARRAY *pa);
extern "C" int ptarray_is_closed_z(const POINTARRAY *pa);
extern "C" POINTARRAY* ptarray_flip_coordinates(POINTARRAY *pa);
 ...
 */
extern "C" POINTARRAY *ptarray_substring(POINTARRAY *pa, double d1, double d2,
                                                          double tolerance);
 ...
 */
extern "C" int ptarray_closest_vertex_2d(const POINTARRAY *pa,
 ...
 */
extern "C" int ptarray_closest_segment_2d(const POINTARRAY *pa,
                                      const POINT2D *qp, double *dist);
...
*/
extern "C" LWGEOM* lwgeom_force_2d(const LWGEOM *geom);
extern "C" LWGEOM* lwgeom_force_3dz(const LWGEOM *geom, double zval);
extern "C" LWGEOM* lwgeom_force_3dm(const LWGEOM *geom, double mval);
extern "C" LWGEOM* lwgeom_force_4d(const LWGEOM *geom, double zval, double mval);

extern "C" LWGEOM* lwgeom_set_effective_area(const LWGEOM *igeom, int set_area, double area);
extern "C" LWGEOM* lwgeom_chaikin(const LWGEOM *igeom, int n_iterations, int preserve_endpoint);
extern "C" LWGEOM* lwgeom_filter_m(LWGEOM *geom, double min, double max, int returnm);

/*
 * Force to use SFS 1.1 geometry type
 * (rather than SFS 1.2 and/or SQL/MM)
 */
extern "C" LWGEOM* lwgeom_force_sfs(LWGEOM *geom, int version);
 ...
 */
extern "C" int lwpoint_getPoint2d_p(const LWPOINT *point, POINT2D *out);
extern "C" int lwpoint_getPoint3dz_p(const LWPOINT *point, POINT3DZ *out);
extern "C" int lwpoint_getPoint3dm_p(const LWPOINT *point, POINT3DM *out);
extern "C" int lwpoint_getPoint4d_p(const LWPOINT *point, POINT4D *out);
 ...
 */
extern "C" int lwline_add_lwpoint(LWLINE *line, LWPOINT *point, uint32_t where);

/**
 * Interpolate one or more points along a line
 */
extern "C" POINTARRAY* lwline_interpolate_points(const LWLINE *line, double length_fraction, char repeat);

/**
 * Interpolate one point along a line in 3D
 */
extern "C" LWPOINT* lwline_interpolate_point_3d(const LWLINE *line, double distance);
...
*/
extern "C" int lwpoly_add_ring(LWPOLY *poly, POINTARRAY *pa);

/**
* Add a ring, allocating extra space if necessary. The curvepolygon takes
* ownership of the passed point array.
*/
extern "C" int lwcurvepoly_add_ring(LWCURVEPOLY *poly, LWGEOM *ring);

/**
* Add a component, allocating extra space if necessary. The compoundcurve
* takes owership of the passed geometry.
*/
extern "C" int lwcompound_add_lwgeom(LWCOMPOUND *comp, LWGEOM *geom);

/**
* Construct an equivalent compound curve from a linestring.
* Compound curves can have linear components, so this works fine
*/
extern "C" LWCOMPOUND* lwcompound_construct_from_lwline(const LWLINE *lwpoly);
...
*/
extern "C" LWCURVEPOLY* lwcurvepoly_construct_from_lwpoly(LWPOLY *lwpoly);
...
*/
extern "C" void lwgeom_set_srid(LWGEOM *geom, int32_t srid);
...
extern "C" BOX3D* box3d_from_gbox(const GBOX *gbox);
extern "C" GBOX* box3d_to_gbox(const BOX3D *b3d);
...
extern "C" void ptarray_free(POINTARRAY *pa);
extern "C" void lwpoint_free(LWPOINT *pt);
extern "C" void lwline_free(LWLINE *line);
extern "C" void lwpoly_free(LWPOLY *poly);
extern "C" void lwtriangle_free(LWTRIANGLE *triangle);
extern "C" void lwmpoint_free(LWMPOINT *mpt);
extern "C" void lwmline_free(LWMLINE *mline);
extern "C" void lwmpoly_free(LWMPOLY *mpoly);
extern "C" void lwpsurface_free(LWPSURFACE *psurf);
extern "C" void lwtin_free(LWTIN *tin);
...
extern "C" void lwbezier_free(LWBEZIER* e);
...
extern "C" void lwpoint_release(LWPOINT *lwpoint);
extern "C" void lwline_release(LWLINE *lwline);
extern "C" void lwpoly_release(LWPOLY *lwpoly);
extern "C" void lwtriangle_release(LWTRIANGLE *lwtriangle);
extern "C" void lwcircstring_release(LWCIRCSTRING *lwcirc);
extern "C" void lwmpoint_release(LWMPOINT *lwpoint);
extern "C" void lwmline_release(LWMLINE *lwline);
extern "C" void lwmpoly_release(LWMPOLY *lwpoly);
extern "C" void lwpsurface_release(LWPSURFACE *lwpsurface);
extern "C" void lwtin_release(LWTIN *lwtin);
extern "C" void lwcollection_release(LWCOLLECTION *lwcollection);
extern "C" void lwgeom_release(LWGEOM *lwgeom);
...
extern "C" void printBOX3D(BOX3D *b);
extern "C" void printPA(POINTARRAY *pa);
extern "C" void printLWPOINT(LWPOINT *point);
extern "C" void printLWLINE(LWLINE *line);
extern "C" void printLWPOLY(LWPOLY *poly);
extern "C" void printLWTRIANGLE(LWTRIANGLE *triangle);
extern "C" void printLWPSURFACE(LWPSURFACE *psurf);
extern "C" void printLWTIN(LWTIN *tin);

extern "C" float  next_float_down(double d);
extern "C" float  next_float_up(double d);

/* general utilities 2D */
extern "C" double  distance2d_pt_pt(const POINT2D *p1, const POINT2D *p2);
extern "C" double  distance2d_sqr_pt_seg(const POINT2D *p, const POINT2D *A, const POINT2D *B);
extern "C" LWGEOM* lwgeom_closest_line(const LWGEOM *lw1, const LWGEOM *lw2);
extern "C" LWGEOM* lwgeom_furthest_line(const LWGEOM *lw1, const LWGEOM *lw2);
extern "C" LWGEOM* lwgeom_closest_point(const LWGEOM *lw1, const LWGEOM *lw2);
extern "C" LWGEOM* lwgeom_furthest_point(const LWGEOM *lw1, const LWGEOM *lw2);
extern "C" double  lwgeom_mindistance2d(const LWGEOM *lw1, const LWGEOM *lw2);
extern "C" double  lwgeom_mindistance2d_tolerance(const LWGEOM *lw1, const LWGEOM *lw2, double tolerance);
extern "C" double  lwgeom_maxdistance2d(const LWGEOM *lw1, const LWGEOM *lw2);
extern "C" double  lwgeom_maxdistance2d_tolerance(const LWGEOM *lw1, const LWGEOM *lw2, double tolerance);

/* 3D */
extern "C" double distance3d_pt_pt(const POINT3D *p1, const POINT3D *p2);
extern "C" double distance3d_pt_seg(const POINT3D *p, const POINT3D *A, const POINT3D *B);

extern "C" LWGEOM* lwgeom_furthest_line_3d(LWGEOM *lw1, LWGEOM *lw2);
extern "C" LWGEOM* lwgeom_closest_line_3d(const LWGEOM *lw1, const LWGEOM *lw2);
extern "C" LWGEOM* lwgeom_closest_point_3d(const LWGEOM *lw1, const LWGEOM *lw2);


extern "C" double lwgeom_mindistance3d(const LWGEOM *lw1, const LWGEOM *lw2);
extern "C" double lwgeom_mindistance3d_tolerance(const LWGEOM *lw1, const LWGEOM *lw2, double tolerance);
extern "C" double lwgeom_maxdistance3d(const LWGEOM *lw1, const LWGEOM *lw2);
extern "C" double lwgeom_maxdistance3d_tolerance(const LWGEOM *lw1, const LWGEOM *lw2, double tolerance);

extern "C" double lwgeom_area(const LWGEOM *geom);
extern "C" double lwgeom_length(const LWGEOM *geom);
extern "C" double lwgeom_length_2d(const LWGEOM *geom);
extern "C" double lwgeom_perimeter(const LWGEOM *geom);
extern "C" double lwgeom_perimeter_2d(const LWGEOM *geom);
extern "C" int lwgeom_dimension(const LWGEOM *geom);

extern "C" LWPOINT* lwline_get_lwpoint(const LWLINE *line, uint32_t where);
extern "C" LWPOINT* lwcircstring_get_lwpoint(const LWCIRCSTRING *circ, uint32_t where);

extern "C" LWPOINT* lwcompound_get_startpoint(const LWCOMPOUND *lwcmp);
extern "C" LWPOINT* lwcompound_get_endpoint(const LWCOMPOUND *lwcmp);
extern "C" LWPOINT* lwcompound_get_lwpoint(const LWCOMPOUND *lwcmp, uint32_t where);

extern "C" double ptarray_length_2d(const POINTARRAY *pts);
extern "C" int pt_in_ring_2d(const POINT2D *p, const POINTARRAY *ring);
extern "C" int azimuth_pt_pt(const POINT2D *p1, const POINT2D *p2, double *ret);
extern "C" int lwpoint_inside_circle(const LWPOINT *p, double cx, double cy, double rad);

extern "C" LWGEOM* lwgeom_reverse(const LWGEOM *lwgeom);
extern "C" char* lwgeom_summary(const LWGEOM *lwgeom, int offset);
extern "C" char* lwpoint_to_latlon(const LWPOINT *p, const char *format);
extern "C" int lwgeom_startpoint(const LWGEOM* lwgeom, POINT4D* pt);

extern "C" void interpolate_point4d(const POINT4D *A, const POINT4D *B, POINT4D *I, double F);

/**
* Ensure the outer ring is clockwise oriented and all inner rings
* are counter-clockwise.
*/
extern "C" int lwgeom_is_clockwise(LWGEOM *lwgeom);


/**
* Simplification
*/
extern "C" LWGEOM* lwgeom_simplify(const LWGEOM *igeom, double dist, int preserve_collapsed);
extern "C" LWGEOM* lwgeom_remove_repeated_points(const LWGEOM *in, double tolerance);
...
extern "C" LWGEOM* lwgeom_grid(const LWGEOM *lwgeom, const gridspec *grid);
extern "C" void lwgeom_grid_in_place(LWGEOM *lwgeom, const gridspec *grid);
...
extern "C" void lwgeom_reverse_in_place(LWGEOM *lwgeom);
extern "C" void lwgeom_force_clockwise(LWGEOM *lwgeom);
extern "C" void lwgeom_longitude_shift(LWGEOM *lwgeom);
extern "C" int lwgeom_simplify_in_place(LWGEOM *igeom, double dist, int preserve_collapsed);
extern "C" void lwgeom_affine(LWGEOM *geom, const AFFINE *affine);
extern "C" void lwgeom_scale(LWGEOM *geom, const POINT4D *factors);
extern "C" int lwgeom_remove_repeated_points_in_place(LWGEOM *in, double tolerance);
...
* when scanning keys.
*/
extern "C" int lwgeom_needs_bbox(const LWGEOM *geom);

/**
* Count the total number of vertices in any #LWGEOM.
*/
extern "C" uint32_t lwgeom_count_vertices(const LWGEOM *geom);
...
*/
extern "C" uint32_t lwgeom_count_rings(const LWGEOM *geom);

/**
* Return true or false depending on whether a geometry has
* a valid SRID set.
*/
extern "C" int lwgeom_has_srid(const LWGEOM *geom);

/**
* Return true or false depending on whether a geometry is a linear
* feature that closes on itself.
*/
extern "C" int lwgeom_is_closed(const LWGEOM *geom);

/**
* Return the dimensionality (relating to point/line/poly) of an lwgeom
*/
extern "C" int lwgeom_dimensionality(const LWGEOM *geom);

/* Is lwgeom1 geometrically equal to lwgeom2 ? */
extern "C" char lwgeom_same(const LWGEOM *lwgeom1, const LWGEOM *lwgeom2);
 ...
 * @see ptarray_clone
 */
extern "C" LWGEOM *lwgeom_clone(const LWGEOM *lwgeom);

/**
* Deep clone an LWGEOM, everything is copied
*/
extern "C" LWGEOM *lwgeom_clone_deep(const LWGEOM *lwgeom);
extern "C" POINTARRAY *ptarray_clone_deep(const POINTARRAY *ptarray);
...
*/
extern "C" LWPOINT* lwpoint_construct(int32_t srid, GBOX *bbox, POINTARRAY *point);
extern "C" LWMPOINT *lwmpoint_construct(int32_t srid, const POINTARRAY *pa);
extern "C" LWLINE* lwline_construct(int32_t srid, GBOX *bbox, POINTARRAY *points);
extern "C" LWCIRCSTRING* lwcircstring_construct(int32_t srid, GBOX *bbox, POINTARRAY *points);
extern "C" LWPOLY* lwpoly_construct(int32_t srid, GBOX *bbox, uint32_t nrings, POINTARRAY **points);
extern "C" LWCURVEPOLY* lwcurvepoly_construct(int32_t srid, GBOX *bbox, uint32_t nrings, LWGEOM **geoms);
extern "C" LWTRIANGLE* lwtriangle_construct(int32_t srid, GBOX *bbox, POINTARRAY *points);
extern "C" LWCOLLECTION* lwcollection_construct(uint8_t type, int32_t srid, GBOX *bbox, uint32_t ngeoms, LWGEOM **geoms);
/*
* Empty geometry constructors.
*/
extern "C" LWGEOM* lwgeom_construct_empty(uint8_t type, int32_t srid, char hasz, char hasm);
extern "C" LWPOINT* lwpoint_construct_empty(int32_t srid, char hasz, char hasm);
extern "C" LWLINE* lwline_construct_empty(int32_t srid, char hasz, char hasm);
extern "C" LWPOLY* lwpoly_construct_empty(int32_t srid, char hasz, char hasm);
extern "C" LWCURVEPOLY* lwcurvepoly_construct_empty(int32_t srid, char hasz, char hasm);
extern "C" LWCIRCSTRING* lwcircstring_construct_empty(int32_t srid, char hasz, char hasm);
extern "C" LWCOMPOUND* lwcompound_construct_empty(int32_t srid, char hasz, char hasm);
extern "C" LWTRIANGLE* lwtriangle_construct_empty(int32_t srid, char hasz, char hasm);
extern "C" LWMPOINT* lwmpoint_construct_empty(int32_t srid, char hasz, char hasm);
extern "C" LWMLINE* lwmline_construct_empty(int32_t srid, char hasz, char hasm);
extern "C" LWMPOLY* lwmpoly_construct_empty(int32_t srid, char hasz, char hasm);
extern "C" LWCOLLECTION* lwcollection_construct_empty(uint8_t type, int32_t srid, char hasz, char hasm);


/* Other constructors */
extern "C" LWPOINT *lwpoint_make2d(int32_t srid, double x, double y);
extern "C" LWPOINT *lwpoint_make3dz(int32_t srid, double x, double y, double z);
extern "C" LWPOINT *lwpoint_make3dm(int32_t srid, double x, double y, double m);
extern "C" LWPOINT *lwpoint_make4d(int32_t srid, double x, double y, double z, double m);
extern "C" LWPOINT *lwpoint_make(int32_t srid, int hasz, int hasm, const POINT4D *p);
extern "C" LWLINE *lwline_from_lwgeom_array(int32_t srid, uint32_t ngeoms, LWGEOM **geoms);
extern "C" LWLINE *lwline_from_ptarray(int32_t srid, uint32_t npoints, LWPOINT **points); /* TODO: deprecate */
extern "C" LWLINE *lwline_from_lwmpoint(int32_t srid, const LWMPOINT *mpoint);
extern "C" LWLINE *lwline_addpoint(LWLINE *line, LWPOINT *point, uint32_t where);
extern "C" LWLINE *lwline_removepoint(LWLINE *line, uint32_t which);
extern "C" void lwline_setPoint4d(LWLINE *line, uint32_t which, POINT4D *newpoint);
extern "C" LWPOLY *lwpoly_from_lwlines(const LWLINE *shell, uint32_t nholes, const LWLINE **holes);
extern "C" LWPOLY *lwpoly_construct_rectangle(char hasz, char hasm, POINT4D *p1, POINT4D *p2, POINT4D *p3, POINT4D *p4);
extern "C" LWPOLY *lwpoly_construct_envelope(int32_t srid, double x1, double y1, double x2, double y2);
extern "C" LWPOLY *lwpoly_construct_circle(int32_t srid, double x, double y, double radius, uint32_t segments_per_quarter, char exterior);
extern "C" LWTRIANGLE *lwtriangle_from_lwline(const LWLINE *shell);
extern "C" LWMPOINT *lwmpoint_from_lwgeom(const LWGEOM *g); /* Extract the coordinates of an LWGEOM into an LWMPOINT */

/* Some point accessors */
extern "C" double lwpoint_get_x(const LWPOINT *point);
extern "C" double lwpoint_get_y(const LWPOINT *point);
extern "C" double lwpoint_get_z(const LWPOINT *point);
extern "C" double lwpoint_get_m(const LWPOINT *point);

/**
* Return SRID number
*/
extern "C" int32_t lwgeom_get_srid(const LWGEOM *geom);

/**
* Return #LW_TRUE if geometry has Z ordinates
*/
extern "C" int lwgeom_has_z(const LWGEOM *geom);

/**
* Return #LW_TRUE if geometry has M ordinates.
*/
extern "C" int lwgeom_has_m(const LWGEOM *geom);

/**
* Return #LW_TRUE if geometry has SOLID flag.
*/
extern "C" int lwgeom_is_solid(const LWGEOM *geom);

/**
* Return the number of dimensions (2, 3, 4) in a geometry
*/
extern "C" int lwgeom_ndims(const LWGEOM *geom);
 ...
 */
extern "C" double ptarray_locate_point(const POINTARRAY *pa, const POINT4D *pt, double *dist, POINT4D *p_located);

/**
* Add a measure dimension to a line, interpolating linearly from the start
* to the end value.
*/
extern "C" LWLINE *lwline_measured_from_lwline(const LWLINE *lwline, double m_start, double m_end);
extern "C" LWMLINE* lwmline_measured_from_lwmline(const LWMLINE *lwmline, double m_start, double m_end);
...
*/
extern "C" LWGEOM* lwgeom_locate_along(const LWGEOM *lwin, double m, double offset);
...
*/
extern "C" LWCOLLECTION* lwgeom_locate_between(const LWGEOM *lwin, double from, double to, double offset);

/**
* Find the measure value at the location on the line closest to the point.
*/
extern "C" double lwgeom_interpolate_point(const LWGEOM *lwin, const LWPOINT *lwpt);
...
*/
extern "C" double lwgeom_tcpa(const LWGEOM *g1, const LWGEOM *g2, double *mindist);
...
*/
extern "C" int lwgeom_cpa_within(const LWGEOM *g1, const LWGEOM *g2, double maxdist);
...
*/
extern "C" int lwgeom_is_trajectory(const LWGEOM *geom);
extern "C" int lwline_is_trajectory(const LWLINE *geom);

/*
 * Ensure every segment is at most 'dist' long.
 * Returned LWGEOM might is unchanged if a POINT.
 */
extern "C" LWGEOM *lwgeom_segmentize2d(const LWGEOM *line, double dist);
extern "C" POINTARRAY *ptarray_segmentize2d(const POINTARRAY *ipa, double dist);
extern "C" LWLINE *lwline_segmentize2d(const LWLINE *line, double dist);
extern "C" LWPOLY *lwpoly_segmentize2d(const LWPOLY *line, double dist);
extern "C" LWCOLLECTION *lwcollection_segmentize2d(const LWCOLLECTION *coll, double dist);

/*
 * Point density functions
 */
extern "C" LWMPOINT *lwpoly_to_points(const LWPOLY *poly, uint32_t npoints, int32_t seed);
extern "C" LWMPOINT *lwmpoly_to_points(const LWMPOLY *mpoly, uint32_t npoints, int32_t seed);
extern "C" LWMPOINT *lwgeom_to_points(const LWGEOM *lwgeom, uint32_t npoints, int32_t seed);

/*
 * Geometric median
 */
extern "C" LWPOINT* lwgeom_median(const LWGEOM *g, double tol, uint32_t maxiter, char fail_if_not_converged);
extern "C" LWPOINT* lwmpoint_median(const LWMPOINT *g, double tol, uint32_t maxiter, char fail_if_not_converged);
...
*/
extern "C" LWCOLLECTION* lwgeom_clip_to_ordinate_range(const LWGEOM *lwin, char ordinate, double from, double to, double offset);
...
extern "C" lwvarlena_t* lwgeom_to_gml2(const LWGEOM *geom, const char *srs, int precision, const char *prefix);
extern "C" lwvarlena_t* lwgeom_extent_to_gml2(const LWGEOM *geom, const char *srs, int precision, const char *prefix);
/**
 * @param opts output options bitfield, see LW_GML macros for meaning
 */
extern "C" lwvarlena_t* lwgeom_extent_to_gml3(const LWGEOM *geom, const char *srs, int precision, int opts, const char *prefix);
extern "C" lwvarlena_t* lwgeom_to_gml3(const LWGEOM *geom, const char *srs, int precision, int opts, const char *prefix, const char *id);
extern "C" lwvarlena_t* lwgeom_to_kml2(const LWGEOM *geom, int precision, const char *prefix);
extern "C" lwvarlena_t* lwgeom_to_geojson(const LWGEOM *geo, const char *srs, int precision, int has_bbox);
extern "C" lwvarlena_t* lwgeom_to_x3d3(const LWGEOM *geom, int precision, int opts, const char *defid);
extern "C" lwvarlena_t* lwgeom_to_svg(const LWGEOM *geom, int precision, int relative);
extern "C" lwvarlena_t* lwgeom_to_encoded_polyline(const LWGEOM *geom, int precision);
 ...
 */
extern "C" LWGEOM* lwgeom_from_geojson(const char *geojson, char **srs);
 ...
 */
extern "C" LWGEOM* lwgeom_from_encoded_polyline(const char *encodedpolyline, int precision);

/**
* Initialize a spheroid object for use in geodetic functions.
*/
extern "C" void spheroid_init(SPHEROID *s, double a, double b);
...
*/
extern "C" double lwgeom_distance_spheroid(const LWGEOM *lwgeom1, const LWGEOM *lwgeom2, const SPHEROID *spheroid, double tolerance);

/**
* Calculate the location of a point on a spheroid, give a start point, bearing and distance.
*/
extern "C" LWPOINT* lwgeom_project_spheroid(const LWPOINT *r, const SPHEROID *spheroid, double distance, double azimuth);
...
*/
extern "C" LWGEOM* lwgeom_segmentize_sphere(const LWGEOM *lwg_in, double max_seg_length);

/**
* Calculate the bearing between two points on a spheroid.
*/
extern "C" double lwgeom_azumith_spheroid(const LWPOINT *r, const LWPOINT *s, const SPHEROID *spheroid);
...
*/
extern "C" double lwgeom_area_sphere(const LWGEOM *lwgeom, const SPHEROID *spheroid);

/**
* Calculate the geodetic area of a lwgeom on the spheroid. The result
* will have the squared units of the spheroid axes.
*/
extern "C" double lwgeom_area_spheroid(const LWGEOM *lwgeom, const SPHEROID *spheroid);
...
*/
extern "C" double lwgeom_length_spheroid(const LWGEOM *geom, const SPHEROID *s);

/**
* Calculate covers predicate for two lwgeoms on the sphere. Currently
* only handles point-in-polygon.
*/
extern "C" int lwgeom_covers_lwgeom_sphere(const LWGEOM *lwgeom1, const LWGEOM *lwgeom2);
...
extern "C" void lwboundingcircle_destroy(LWBOUNDINGCIRCLE* c);
 ...
 */
extern "C" LWBOUNDINGCIRCLE* lwgeom_calculate_mbc(const LWGEOM* g);
 ...
 * Availability: 2.2.0
 */
extern "C" void lwgeom_swap_ordinates(LWGEOM *in, LWORD o1, LWORD o2);
 ...
 */
extern "C" LWPOINTITERATOR* lwpointiterator_create(const LWGEOM* g);

/**
 * Create a new LWPOINTITERATOR over supplied LWGEOM*
 * Supports modification of coordinates during iteration.
 */
extern "C" LWPOINTITERATOR* lwpointiterator_create_rw(LWGEOM* g);

/**
 * Free all memory associated with the iterator
 */
extern "C" void lwpointiterator_destroy(LWPOINTITERATOR* s);

/**
 * Returns LW_TRUE if there is another point available in the iterator.
 */
extern "C" int lwpointiterator_has_next(LWPOINTITERATOR* s);
 ...
 * */
extern "C" int lwpointiterator_modify_next(LWPOINTITERATOR* s, const POINT4D* p);
 ...
 * Returns LW_SUCCESS if the assignment was successful, LW_FAILURE otherwise.
 * */
extern "C" int lwpointiterator_next(LWPOINTITERATOR* s, POINT4D* p);

/**
 * Attempts to assigns the next point in the iterator to p.  Does not advance.
 * Returns LW_SUCCESS if the assignment was successful, LW_FAILURE otherwise.
 */
extern "C" int lwpointiterator_peek(LWPOINTITERATOR* s, POINT4D* p);


/**
* Convert a single hex digit into the corresponding char
*/
extern "C" uint8_t parse_hex(char *str);

/**
* Convert a char into a human readable hex digit
*/
extern "C" void deparse_hex(uint8_t str, char *result);
...
*/
extern "C" int lwgeom_check_geodetic(const LWGEOM *geom);

/**
* Gently move coordinates of LWGEOM if they are close enough into geodetic range.
*/
extern "C" int lwgeom_nudge_geodetic(LWGEOM *geom);

/**
* Force coordinates of LWGEOM into geodetic range (-180, -90, 180, 90)
*/
extern "C" int lwgeom_force_geodetic(LWGEOM *geom);

/**
* Set the FLAGS geodetic bit on geometry an all sub-geometries and pointlists
*/
extern "C" void lwgeom_set_geodetic(LWGEOM *geom, int value);
...
*/
extern "C" int lwgeom_calculate_gbox_geodetic(const LWGEOM *geom, GBOX *gbox);
...
* of curvature.
*/
extern "C" int lwgeom_calculate_gbox_cartesian(const LWGEOM *lwgeom, GBOX *gbox);

/**
* Calculate bounding box of a geometry, automatically taking into account
* whether it is cartesian or geodetic.
*/
extern "C" int lwgeom_calculate_gbox(const LWGEOM *lwgeom, GBOX *gbox);

/**
* Calculate geodetic (x/y/z) box and add values to gbox. Return #LW_SUCCESS on success.
*/
extern "C" int ptarray_calculate_gbox_geodetic(const POINTARRAY *pa, GBOX *gbox);

/**
* Calculate box (x/y) and add values to gbox. Return #LW_SUCCESS on success.
*/
extern "C" int ptarray_calculate_gbox_cartesian(const POINTARRAY *pa, GBOX *gbox );
...
*/
extern "C" GBOX* gbox_new(lwflags_t flags);

/**
* Zero out all the entries in the #GBOX. Useful for cleaning
* statically allocated gboxes.
*/
extern "C" void gbox_init(GBOX *gbox);

/**
* Update the merged #GBOX to be large enough to include itself and the new box.
*/
extern "C" int gbox_merge(const GBOX *new_box, GBOX *merged_box);

/**
* Update the output #GBOX to be large enough to include both inputs.
*/
extern "C" int gbox_union(const GBOX *g1, const GBOX *g2, GBOX *gout);

/**
* Move the box minimums down and the maximums up by the distance provided.
*/
extern "C" void gbox_expand(GBOX *g, double d);

/**
* Move the box minimums down and the maximums up by the distances provided.
*/
extern "C" void gbox_expand_xyzm(GBOX *g, double dx, double dy, double dz, double dm);

/**
* Initialize a #GBOX using the values of the point.
*/
extern "C" int gbox_init_point3d(const POINT3D *p, GBOX *gbox);

/**
* Update the #GBOX to be large enough to include itself and the new point.
*/
extern "C" int gbox_merge_point3d(const POINT3D *p, GBOX *gbox);

/**
* Return true if the point is inside the gbox
*/
extern "C" int gbox_contains_point3d(const GBOX *gbox, const POINT3D *pt);

/**
* Allocate a string representation of the #GBOX, based on dimensionality of flags.
*/
extern "C" char* gbox_to_string(const GBOX *gbox);

/**
* Return a copy of the #GBOX, based on dimensionality of flags.
*/
extern "C" GBOX* gbox_copy(const GBOX *gbox);

/**
* Warning, do not use this function, it is very particular about inputs.
*/
extern "C" GBOX* gbox_from_string(const char *str);

/**
* Return #LW_TRUE if the #GBOX overlaps, #LW_FALSE otherwise.
*/
extern "C" int gbox_overlaps(const GBOX *g1, const GBOX *g2);

/**
* Return #LW_TRUE if the #GBOX overlaps on the 2d plane, #LW_FALSE otherwise.
*/
extern "C" int gbox_overlaps_2d(const GBOX *g1, const GBOX *g2);

/**
* Return #LW_TRUE if the first #GBOX contains the second on the 2d plane, #LW_FALSE otherwise.
*/
extern "C" int  gbox_contains_2d(const GBOX *g1, const GBOX *g2);

/**
* Copy the values of original #GBOX into duplicate.
*/
extern "C" void gbox_duplicate(const GBOX *original, GBOX *duplicate);

/**
* Return the number of bytes necessary to hold a #GBOX of this dimension in
* serialized form.
*/
extern "C" size_t gbox_serialized_size(lwflags_t flags);

/**
* Check if 2 given Gbox are the same
*/
extern "C" int gbox_same(const GBOX *g1, const GBOX *g2);

/**
* Check if 2 given GBOX are the same in x and y
*/
extern "C" int gbox_same_2d(const GBOX *g1, const GBOX *g2);

/**
* Check if two given GBOX are the same in x and y, or would round to the same
* GBOX in x and if serialized in GSERIALIZED
*/
extern "C" int gbox_same_2d_float(const GBOX *g1, const GBOX *g2);
 ...
 */
extern "C" void gbox_float_round(GBOX *gbox);

/**
* Return false if any of the dimensions is NaN or infinite
*/
extern "C" int gbox_is_valid(const GBOX *gbox);

/**
* Return a sortable key based on the center point of the GBOX.
*/
extern "C" uint64_t gbox_get_sortable_hash(const GBOX *g, const int32_t srid);

/**
* Return a sortable key based on gserialized.
*/
extern "C" uint64_t gserialized_get_sortable_hash(const GSERIALIZED *g);
...
*/
extern "C" int geometry_type_from_string(const char *str, uint8_t *type, int *z, int *m);
...
*/
extern "C" char*   lwgeom_to_wkt(const LWGEOM *geom, uint8_t variant, int precision, size_t *size_out);
...
* @param precision Double precision
*/
extern "C" lwvarlena_t* lwgeom_to_wkt_varlena(const LWGEOM *geom, uint8_t variant, int precision);
...
*/
extern "C" uint8_t* lwgeom_to_wkb_buffer(const LWGEOM *geom, uint8_t variant);
extern "C" lwvarlena_t* lwgeom_to_wkb_varlena(const LWGEOM *geom, uint8_t variant);
...
* @param size_out (Out parameter) size of the buffer
*/
extern "C" char* lwgeom_to_hexwkb_buffer(const LWGEOM *geom, uint8_t variant);
extern "C" lwvarlena_t* lwgeom_to_hexwkb_varlena(const LWGEOM *geom, uint8_t variant);

/**
* @param lwgeom geometry to convert to EWKT
*/
extern "C" char *lwgeom_to_ewkt(const LWGEOM *lwgeom);
 ...
 */
extern "C" LWGEOM* lwgeom_from_wkb(const uint8_t *wkb, const size_t wkb_size, const char check);

/**
 * @param wkt WKT string
 * @param check parser check flags, see LW_PARSER_CHECK_* macros
 */
extern "C" LWGEOM* lwgeom_from_wkt(const char *wkt, const char check);

/**
 * @param check parser check flags, see LW_PARSER_CHECK_* macros
 */
extern "C" LWGEOM* lwgeom_from_hexwkb(const char *hexwkb, const char check);

extern "C" uint8_t*  bytes_from_hexbytes(const char *hexbuf, size_t hexsize);

extern "C" char*   hexbytes_from_bytes(const uint8_t *bytes, size_t size);

/*
* WKT detailed parsing support
*/
extern "C" int lwgeom_parse_wkt(LWGEOM_PARSER_RESULT *parser_result, char *wktstr, int parse_flags);
void lwgeom_parser_result_init(LWGEOM_PARSER_RESULT *parser_result);
void lwgeom_parser_result_free(LWGEOM_PARSER_RESULT *parser_result);


/* Memory management */
extern "C" void *lwalloc(size_t size);
extern "C" void *lwrealloc(void *mem, size_t size);
extern "C" void lwfree(void *mem);

/* Utilities */
extern "C" char *lwmessage_truncate(char *str, int startpos, int endpos, int maxlength, int truncdirection);
 ...
 * @param check parser check flags, see LW_PARSER_CHECK_* macros
 */
extern "C" LWGEOM* lwgeom_from_twkb(const uint8_t *twkb, size_t twkb_size, char check);
 ...
 */
extern "C" lwvarlena_t* lwgeom_to_twkb(const LWGEOM *geom, uint8_t variant, int8_t precision_xy, int8_t precision_z, int8_t precision_m);

extern "C" lwvarlena_t* lwgeom_to_twkb_with_idlist(const LWGEOM *geom, int64_t *idlist, uint8_t variant, int8_t precision_xy, int8_t precision_z, int8_t precision_m);
 ...
 */
extern "C" void lwgeom_trim_bits_in_place(LWGEOM *geom, int32_t prec_x, int32_t prec_y, int32_t prec_z, int32_t prec_m);

extern "C" LWGEOM* lwgeom_boundary(LWGEOM* lwgeom);
 ...
 */
extern "C" LWGEOM* lwcurve_linearize(const LWGEOM *geom, double tol, LW_LINEARIZE_TOLERANCE_TYPE type, int flags);
